...
// DOM Elements (may be null on some pages, so always check before use)
...
function initMap() {
    ...
    // Default center (Sri Lanka center as fallback)
...
function getUserLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // Recalculate distances and filters if salons already loaded
...
async function loadAllSalons() {
    try {
        ...
        if (!Array.isArray(data)) {
            console.error('Invalid response format from get_all_salons.php:', data);
            return;
        }
...
function applyFilters() {
    filteredSalons = allSalons.filter(salon => {
        // High rated filter
        if (filterHighRated && filterHighRated.checked && salon.rating != null && salon.rating < 4.0) {
            return false;
        }

        // Nearby filter (within 10km)
        if (filterNearby && filterNearby.checked && userLocation && salon.distance != null && salon.distance > 10) {
            return false;
        }

        // Open now filter (uses opening_time and closing_time from DB)
        if (filterOpenNow && filterOpenNow.checked && !isOpenNow(salon.opening_time, salon.closing_time)) {
            return false;
        }
    ...
    if (searchStatsDisplay && resultCount) {
        if (filteredSalons.length > 0) {
            searchStatsDisplay.style.display = 'block';
            resultCount.textContent = filteredSalons.length;
        } else if (!currentSearchResults.length) {
            // If no active search, we can hide stats
            searchStatsDisplay.style.display = 'none';
        }
    }
}

// Check if salon is open now (uses opening_time & closing_time)
function isOpenNow(openingTime, closingTime) {
    ...
    // openingTime & closingTime expected as "HH:MM:SS" (MySQL TIME)
    const [oh, om] = openingTime.split(':').map(Number);
    const [ch, cm] = closingTime.split(':').map(Number);

    if (isNaN(oh) || isNaN(om) || isNaN(ch) || isNaN(cm)) {
        // If parsing fails, don't block the salon
        return true;
    }
    ...
    // Same-day opening (e.g., 09:00–18:00)
    if (closeMinutes > openMinutes) {
        return currentMinutes >= openMinutes && currentMinutes <= closeMinutes;
    }

    // Overnight opening (e.g., 20:00–02:00)
    return currentMinutes >= openMinutes || currentMinutes <= closeMinutes;
}
...
function updateMapMarkers(salons) {
    ...
    salons.forEach(salon => {
        if (!salon.lat || !salon.lng) return; // skip if no coordinates
...
function getMarkerColor(rating) {
    if (rating == null) return '#6b7280'; // grey
...
function debounceSearch(query) {
    ...
    if (!searchResults || !clearSearchBtn || !searchStatsDisplay) return;
...
function displaySearchResults(results) {
    ...
    if (userLocation) {
        results.forEach(salon => {
            if (!salon.distance && salon.lat && salon.lng) {
                salon.distance = calculateDistance(
                    userLocation.lat,
                    userLocation.lng,
                    salon.lat,
...
function handleSearchResultClick(element) {
    ...
    const salonId = parseInt(element.dataset.salonId, 10);

    if (isNaN(lat) || isNaN(lng) || isNaN(salonId)) {
        console.warn('Invalid salon coordinates or ID on click');
        return;
    }

    console.log('Clicked salon:', salonId, 'at', lat, lng);
    console.log('Available markers:', window.salonMarkers ? Object.keys(window.salonMarkers) : []);
    ...
    // Update search input
    if (searchInput) {
        const salonName = element.querySelector('.search-result-name').childNodes[0].textContent.trim();
...
function updateMapWithSearchResults(results) {
    if (!markerClusterGroup || !Array.isArray(results) || results.length === 0) return;
    ...
    results.forEach(salon => {
        ...
        markerClusterGroup.addLayer(marker);
        window.salonMarkers[salon.id] = marker;
        coords.push([salon.lat, salon.lng]);
    });
...
function clearSearch() {
    ...
    if (searchStatsDisplay) searchStatsDisplay.style.display = 'none';
...
[filterHighRated, filterOpenNow, filterNearby].forEach(filter => {
    ...
    filter.addEventListener('change', () => {
        const query = searchInput ? searchInput.value.trim() : '';
        if (currentSearchResults.length > 0 && query) {
            // If search is active, reapply search with filters
            performSearch(query);
        } else {
